...
$db = mysqli_connect("localhost","root","admin","userdb");
...
function add_cart(){
    ...
    if(isset($_GET['add_cart'])){
        ...
        $check_product = "select * from cart where uid='$uid' AND p_id='$p_id'";

        $run_check = mysqli_query($db,$check_product);

        if(mysqli_num_rows($run_check)>0){

            echo "<script>alert('장바구니에 동일 상품이 존재합니다.')</script>";
            echo "<script>window.open('details.php?pro_id=$p_id','_self')</script>";

        }else{

            echo "<script>alert('추가되었습니다.')</script>";
            $query = "insert into cart (p_id,uid,ip_add,qty,size) values ('$p_id','$uid','$ip_add','$product_qty','$product_size')";
        ...
        while($row_products=mysqli_fetch_array($run_products)){
            ...
            <div class='product'>
                ...
                <a href='details.php?pro_id=$pro_id'>
                    ...
                    <img class='img-responsive' src='admin/product_images/$pro_img1' width='250px' height='250px'>

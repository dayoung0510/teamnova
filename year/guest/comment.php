	...
	<table>
		<tbody style="color: #394C59">
			<tr>
				...
				<td class="commentId" name="coId" id="coId">
<!--                    <input type="text" name="coId" id="coId">-->
			...
			<tr>
				<th scope="row">
                    ...
                    <!-- 비회원 : 비밀번호 라고 써있는 칸 안보이게 - 3월1일 20시30분 -->
                    <?php
                    if(isset($_SESSION['userid'])){ ?>
			...
			<tr>
                ...
                <!-- 비회원 : 내용 입력칸 안보이게 -->
                    <?php
                    if(isset($_SESSION['userid'])){ ?>
                        <td><textarea name="coContent" id="coContent"></textarea>
	...
	<div class="btnSet">
        ...
        <?php
        if(isset($_SESSION['userid'])){ ?>
        <td><input type="submit" value="코멘트 작성">
		...
		$('#commentView').delegate('.comt', 'click', function () {
			...
			if($(this).hasClass('write')) {
				...
				if(action !== 'd') {
					...
					// comment += '				<td>' + coId + '</td>';
                    comment += '<td>    <?php echo "{$_SESSION['userid']}" ?> </td>';
					comment += '			</tr>';
				}
				comment += '			<tr>';
